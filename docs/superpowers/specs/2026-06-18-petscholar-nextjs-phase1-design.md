...
## 後續階段需求補充（backlog，非 Phase 1）

使用者於 2026-06-18 追加的需求，記錄於此避免遺漏：

- **[Phase 4] 寵物愛心值跨頁同步**：舊版各分頁顯示的愛心（HP）值不一致。新架構須將寵物 HP /
  金幣設計為**單一資料來源**（DB 持久化 + 全站共用 state），所有頁面讀同一份、寫入即時同步，
  確保任一頁顯示一致。此原則在設計全站 state 時即納入。
- **[Phase 5] 系統管理後台**：
